...
def _observer(domain: str) -> Callable | None:
...
def _validate_output(service, cycle: dict, output: dict) -> None:
    ...
    try:
        for row in output["understanding"] + output["investigations"]:
            refs = row.get("evidence_ids", []) + row.get("counterevidence_ids", [])
            if not row.get("evidence_ids") or any(ref not in allowed for ref in refs):
                raise LearningError("Cognition cited evidence outside the supplied snapshot")
            service.evidence_records(refs)
            if row in output["understanding"]:
