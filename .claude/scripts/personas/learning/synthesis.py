...
def _journal_sources(service, kind: str) -> list[dict]:
    ...
    rows = []
    # Changes to status/conclusion are revisions too; old invalid copies are
    # visible as counterevidence but never appear as active understanding.
    for record in reversed(service.store.all()):
        ...
        snapshot = {
            ...
            if key
            not in {"source_manifest", "input_manifest", "omitted_manifest", "producer_runtime"}
        }
        ...
        rows.append(
            {
                ...
                "metadata": {
                    ...
                    "source_projection": "journal-record-v1",
                },
            }
        )
    return rows
...
def _inputs_current(service, cycle):
    ...
    for ref, row in derived.items():
        ...
        if not legacy_understanding_current(row, service):
            return False
        if content_hash(row) != manifest.get(ref, {}).get("revision"):
            return False
    return True
