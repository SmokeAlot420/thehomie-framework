...
| Capability Gateway | Read-only six-kind catalog deployed on Homie 1.7.1 | [capability-gateway](features/capability-gateway.md) | `/capabilities`, `/api/capabilities/status` |
...
| **Persona Identity And Prompt Architecture** | Shipped baseline — READ BEFORE authoring persona files or prompting a persona from code: which of the 15 seeded files actually load, region budgets and the 27K clamp, the ordering authority, and where standing behavior belongs vs a per-turn prompt | [persona-identity-and-prompt](features/persona-identity-and-prompt.md) | `SOUL.md`/`SAFETY.md`/`USER.md` authoring, any scheduled job that prompts a persona |
...
| GEO Authority Engine | Implemented, opt-in and exact-approval gated | [geo-authority-cadence](features/geo-authority-cadence.md) | `/signal authority`, `/social outcome`, `SecondBrain-GEOAuthority` |
...
| YourBusiness Fleet GEO Research Control | Private operator control — $25 monthly ceiling, brokered five-brand candidate Google AIO evidence, receipt and no-mutation gates | [seo-geo-fleet-research-control](features/seo-geo-fleet-research-control.md) | `seo_geo`, DataForSEO broker, `fleet-budget`, `fleet-paid-research` |
